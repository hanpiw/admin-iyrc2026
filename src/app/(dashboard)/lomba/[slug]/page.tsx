...
export default function LombaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { peserta, loading, toggleAcc, deletePeserta, refresh } = usePeserta(slug)
  ...
  useEffect(() => {
    async function checkPermission() {
      ...
      if (profile.role === 'super_admin') {
        setCanAddPeserta(true)
      } else if (profile.role === 'pic' && profile.lomba_id === lombaId) {
        setCanAddPeserta(true)
      } else {
        setCanAddPeserta(false)
      }
    }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Kelola peserta dan status verifikasi untuk kategori ini.
          </p>
        </div>
